...
describe("pilot governance contracts", () => {
  it("accepts a closed retention policy and rejects unknown fields", () => {
    ...
    expect(() => pilotRetentionPolicyContract.parse({ ...policy, learnerEmail: "secret" })).toThrow("INVALID_PILOT_RETENTION_POLICY");
  });
  ...
  it("only permits explicit no-persistent-copy authority", () => {
    ...
    expect(() => providerCopyAuthorityContract.parse({ ...authority, secret: "do-not-store" })).toThrow("INVALID_PROVIDER_COPY_AUTHORITY");
    expect(() => providerCopyAuthorityContract.parse({ ...authority, lifecycleMode: "delete_and_probe" })).toThrow("INVALID_PROVIDER_COPY_AUTHORITY");
  });
