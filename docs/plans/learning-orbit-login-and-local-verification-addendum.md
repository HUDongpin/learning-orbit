...
- `/login` is the only unauthenticated rendered product page and form entry for both roles. Its student panel accepts exactly `roomCode` and `seatCode`; its teacher panel accepts an email address and requests the existing passwordless Magic Link. The Magic Link consume URL remains a separate unauthenticated, one-time authentication endpoint, not a rendered product page.
...
- The response uses `Cache-Control: no-store`. No valid session returns `401 {"code":"AUTH_REQUIRED"}`. A student or any other wrong-role principal returns the same privacy-preserving `404 {"code":"ROOM_NOT_FOUND"}` used for unauthorized room discovery. Service failure returns `503 {"code":"ROOM_LIST_UNAVAILABLE"}` and no partial list.
...
The harness owns both application child processes and runs them from a disposable detached Git worktree created at the captured source SHA; the invoking verification worktree remains read-only. It starts Fastify on `http://127.0.0.1:3001`, then starts the installed Next.js 16.2.9 binary with the exact shape `next dev --hostname 127.0.0.1 --port 3000 --experimental-https --experimental-https-key <temporary-key> --experimental-https-cert <temporary-certificate>`. That Next process is the only process that binds port 3000 and owns the TLS certificate, page/asset serving, HTTP proxying, and WebSocket `upgrade` handling. The harness health-checks Fastify directly and Next through the public HTTPS origin, records both child PIDs, and terminates only those children during teardown. No custom or third proxy layer is permitted.

Only when `LO_LOCAL_SAME_ORIGIN_PROXY=1` is set for this local harness may Next.js externally rewrite `/v1/:path*` to `http://127.0.0.1:3001/v1/:path*`. With the flag absent or any value other than the exact string `1`, the rewrite list is empty. Production startup must reject this local-only flag.

The installed Next external-rewrite proxy is the sole owner of ordinary HTTP forwarding and WebSocket upgrades under `/v1/*`. A topology contract test must open `wss://127.0.0.1:3000/v1/rooms/{roomId}/realtime`, observe the Fastify welcome/resume protocol, and fail if the upgrade stays in Next or bypasses port 3000. The rewrite never matches or proxies `/internal`, `/internal/*`, an arbitrary Fastify path, or port 3001 directly. Tests prove the public origin cannot reach an internal route.
...
`SessionGateway` is the Web application's only REST boundary. Every request uses a canonical route builder, and every structured success JSON body uses its generated response type and generated runtime parser. It always sends `credentials: "include"`, uses `cache: "no-store"` for session, list, export, and deletion reads, and never defines a second wire model in `apps/web`. Generated parse failure is a terminal response failure for that operation, not permission to keep unvalidated fields or use a fixture.

The controlled non-JSON success exceptions are explicit: logout has no `204` body, Magic Link consumption is normal browser navigation with a `303` redirect, and export is a bounded binary body plus a strictly parsed `Content-Disposition` header. Error JSON uses a new shared, closed `packages/contracts/schemas/api-error.v1.json` generated union rather than local casts. Its initial allowlist includes at least `AUTH_REQUIRED`, `ROOM_NOT_FOUND`, and `ROOM_LIST_UNAVAILABLE`; endpoint/status tests constrain which code is legal for each response. Unknown codes, extra fields, malformed JSON, or status/code mismatches fail closed.
...
- `LO_STORAGE_BROWSER_ORIGINS` is the single non-secret exact storage-origin allowlist: Fastify validates it and Next injects that same value into the browser bundle. An empty value prevents media dependency composition and media database writes. A provider-enabled build additionally requires an explicitly reviewed complete storage-capability manifest; defaults never assert strong checksum, conditional promotion, or write-once behavior on a provider's behalf.
- A ready attachment is downloadable only from the canonical Plan 02 safe derivative (`sanitized_image` or `playback_audio`), never from the immutable original upload. The server verifies the derivative row, singular room-scoped object key, size, MIME, SHA-256, and strong HEAD result before requesting a signed URL. The signer returns its actual `signedAt` and `expiresAt`; an exact bounded window and clock-skew check must pass before the generated grant can reach the browser. The ready public attachment view projects the same verified derivative size and MIME so the browser can enforce exact bounded-Blob validation without placing the signed URL in the DOM.
...
- `/teacher` lists the authenticated teacher's bounded room summaries and supports creation. Its create-success state immediately displays the one-time room code and four seat codes in that same page, with keyboard-operable copy/print actions. Navigation remains disabled until the teacher explicitly confirms that the codes were saved. Leaving or reloading the page destroys that in-memory result and shows honest copy that the codes cannot be recovered; no query string, history state, Web Storage, room-list request, or room-detail request may rehydrate them. Only after confirmation may the page navigate to `/session/{roomId}/teacher`.
- `/session/{roomId}/teacher` retains Plan 05's owned room controls, lifecycle acknowledgments, Agent policy controls, review/correction queue, ECHO teacher shadow, TRACE teacher bundle, export, and deletion recovery. It does not redisplay one-time access codes. The teacher view has no ordinary student composer and cannot revise a student's words.
...
Individual tests remain useful during development, but their results cannot be combined manually into the local integrated claim. The root command is a fail-fast orchestrator using argument arrays with `shell: false`; it owns startup, readiness, evidence, teardown, and final cleanliness checks. Before any build, generator, Next dev, or E2E command that may write derived files, it creates one run-owned disposable detached Git worktree at the captured SHA, installs only the frozen dependencies there, and executes those commands there. The invoking worktree is never auto-formatted, generated into, reset, checked out, or silently restored by the runner. The disposable path and Git worktree registration are part of the ownership receipt and are removed only after their SHA/path registration matches the current run.

### 5.1 Exact prerequisites and no-skip rule

Before any test process starts, the command must verify:

- `HEAD` resolves to a commit, the worktree is clean, and the runner captures its full SHA plus either the current branch name or an explicit detached-HEAD marker; branch and detached checkouts are both eligible when the start and end SHA are identical;
...
A missing runtime, browser, PostgreSQL 18 service, migration, certificate tool, provider fail-closed seam, or required test is a failure with a stable redacted code. It is never converted to a skipped, pending, or advisory pass.

The checked-in closed manifest `tests/pilot/required-test-manifest.v1.json` owns the required selection. It has `schemaVersion: 1` and a non-empty `gates` array whose items contain exactly `id`, `runner`, `argv`, `report`, and positive integer `expectedTests`. The runner values are the closed set `vitest | python-unittest | playwright | load`. Each adapter writes one content-free summary with exactly `expected`, `executed`, `passed`, `failed`, `skipped`, `pending`, and `focused`. A gate passes only when `expected === expectedTests`, `executed === expected > 0`, `passed === executed`, and every other count is zero. Empty or incorrectly filtered runs, `--passWithNoTests`, focused-only cases, missing files/reports, and an unregistered suite all fail the manifest gate.
...
For the teacher journey, local Fastify must use its real SMTP composition to deliver the message to the Compose service named `mailpit`, pinned exactly to `axllent/mailpit:v1.31.0@sha256:c96991d9bef73594c246d89ca81411d4e916f03e76a7d2d72fa2ab5dd3c9ce24`. SMTP uses the isolated service's port 1025; the harness uses port 8025 only from the controlling process, waits for `/readyz`, and never exposes that API through Next. Contract tests freeze the version-pinned Mailpit query, single-message read, and deletion endpoints plus their strict local response parsers. Image-digest drift, an unknown response field/shape, or endpoint mismatch fails as `MAILPIT_CONTRACT_MISMATCH`.

The orchestrator assigns one run-unique synthetic teacher address, polls Mailpit only for that recipient, reads only the matching message in the controlling process, and parses the public `https://127.0.0.1:3000/v1/auth/teacher/magic-link/consume` URL there. The entire token-bearing browser journey runs in a dedicated Playwright context with trace, video, screenshots, request logging, and storage-state export disabled before navigation. Playwright performs ordinary navigation to the URL, lets Fastify consume it, verifies the `303` destination and authenticated teacher state, exercises logout, and destroys the context; it never creates a trace and never persists the cookie. Any later trace-enabled teacher test provisions a fresh session without reusing this Magic Link or its context and may not claim Magic Link coverage. The page never receives a development-only login URL or mail API. The message body, Magic Link, token, recipient address, and resulting cookie must not appear in page state, browser console, application/test logs, command output, or receipts. Docker and Mailpit are required local-gate dependencies: if either is unavailable, unhealthy, or cannot deliver/read the matching message, the gate fails with a redacted reason code; it never falls back to an injected sender, in-process/file link capture, request interception, post-hoc artifact scrubbing, or a skipped test.

The pinned Mailpit client sends `Accept-Encoding: identity` on every API request and rejects any response carrying `Content-Encoding`. This keeps the declared `Content-Length` equal to the bytes exposed to the bounded reader instead of relying on a runtime's automatic decompression behavior.

### 5.3 Cleanup and receipts

Each run creates a unique, validated Docker Compose project name and lets that one project create both its Mailpit service and its own PostgreSQL container, volume, and run-unique database. Before migrations, the harness writes a content-free ownership marker containing the run ID, Compose project label, database name, and creator PID; teardown may act only when the live Compose labels and marker match its in-memory ownership tuple. A pre-existing listener, database, volume, reused project label, or missing/mismatched marker fails preflight. The harness never connects the release gate to a shared database, never performs an unscoped truncate, and never removes a database or volume it did not create.

On success, test failure, timeout, `SIGINT`, or `SIGTERM`, the runner terminates only application child processes it started and closes browsers and sockets while the run-owned Mailpit service remains healthy. It then deletes every Mailpit message addressed to the run-unique synthetic teacher and re-queries Mailpit to verify that no such message remains. Only after that residual check passes may it validate the Compose ownership tuple again and bring down that one project with its PostgreSQL/Mailpit volumes. It then removes the registered disposable Git worktree, deletes the temporary TLS directory, and verifies its ports are released. A Git-worktree, database-ownership, Mailpit-cleanup, residual-message, Compose, or port-release failure invalidates the gate. The runner never kills or adopts an unowned process, never deletes a pre-existing database or user-owned service, and never restores tracked files in the invoking worktree.

Cleanup actions encode that order as conditional dependencies. If an acquired upstream resource cannot be cleaned, every registered destructive downstream action is recorded as a dependency failure and is not executed; resources that were never acquired do not block cleanup of earlier material. The independent final-state check still reports owned residue for explicit recovery.
...
Implementation treats `apps/web/next-env.d.ts` as a Next-generated, ignored, untracked file because `next build` and `next dev` generate different route-type imports. The Web typecheck command must run `next typegen` before `tsc`; teardown must continue rejecting every unexpected tracked change rather than restoring generated drift.
