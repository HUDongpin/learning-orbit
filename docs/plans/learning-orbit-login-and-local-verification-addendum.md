...
For the teacher journey, the local E2E composition uses the existing `buildApp({ sendMagicLink })` port to capture only the synthetic test link in process, or in a mode-`0600` file inside the same temporary directory when a process boundary requires it. The URL is never returned by an app endpoint or exposed to page code; Playwright performs ordinary browser navigation to it. Production composition remains SMTP-backed. The captured link and token are never logged or included in receipts and are deleted during teardown.

### 5.3 Cleanup and receipts

On success, test failure, timeout, `SIGINT`, or `SIGTERM`, the runner terminates only child processes it started, closes browsers and sockets, removes its disposable database/test rows, removes captured synthetic Magic Links and session state, deletes the temporary TLS directory, and verifies its ports are released. A pre-existing listener causes a preflight failure; the runner never kills or adopts an unowned process. It may not delete a pre-existing database or user-owned service.
